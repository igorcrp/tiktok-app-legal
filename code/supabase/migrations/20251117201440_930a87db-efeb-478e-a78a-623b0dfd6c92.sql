-- Drop old table and create new aggregated stats table
...
CREATE TABLE backtest_stats (
  ...
  entry_percent NUMERIC(6,3) NOT NULL,
  stop_percent NUMERIC(6,3) NOT NULL,
  asset_class TEXT,
  exchange TEXT,
  ...
  UNIQUE(asset_code, operation, reference_price, entry_percent, stop_percent)
);

CREATE INDEX idx_backtest_stats_lookup ON backtest_stats(asset_code, operation, entry_percent, stop_percent);

-- Enable RLS
ALTER TABLE backtest_stats ENABLE ROW LEVEL SECURITY;

-- Allow authenticated users to read
CREATE POLICY "Authenticated users can read backtest stats"
ON backtest_stats FOR SELECT
TO authenticated
USING (true);

-- Allow system to insert/update (service role)
CREATE POLICY "System can upsert backtest stats"
ON backtest_stats FOR ALL
TO authenticated
USING (true)
WITH CHECK (true);

-- Create or replace the probability calculation function
